...
```
**Given the total number of steps per day data, the mean is `r meanSPDInt` 
and the median is `r medianSPD`.**
...
```
**The interval with the max number of steps is `r max` with an average value of 
`r numMax`.**
...
not changed and is still `r meanImputedInt`, but the median has slightly changed 
from `r medianSPD` to a new value of `r medianImputedInt`.**
